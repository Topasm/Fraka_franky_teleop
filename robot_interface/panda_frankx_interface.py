...
from _franky import NetworkException
import cv2
from franky import (
    Affine,
    ImpedanceMotion,
...
# franky needs continuous Euler angles around TCP, as the trajectory generation works in the Euler space.
# internally, franky expects orientations with the z-axis facing up, but to be consistent with other
...
class PandafrankyInterface(BaseRobotInterface):
    ...
    (https://github.com/lukashermann/franky)
    ...
    def __init__(
        ...
        # robot
        self.robot = Robot(fci_ip, urdf_path=(
            get_git_root(__file__) / urdf_path).as_posix())
    ...
    def set_robot_params(self, libfranka_params, franky_params):
        ...
        # params of franky
        self.robot.velocity_rel = franky_params.velocity_rel
        self.robot.acceleration_rel = franky_params.acceleration_rel
        self.robot.jerk_rel = franky_params.jerk_rel
    ...
    def move_async_cart_pos_abs_ptp(self, target_pos, target_orn):
        ...
        if self.use_impedance:
            log.warning(
                "Impedance motion for cartesian PTP is currently not implemented. Not using impedance.")
        q_desired = self._inverse_kinematics(target_pos, target_orn)
    ...
    def move_cart_pos_abs_lin(self, target_pos, target_orn):
        ...
        if self.use_impedance:
            log.warning(
                "Impedance motion for cartesian LIN is currently not implemented. Not using impedance.")
        self.abort_motion()
    ...
    def move_async_joint_pos(self, joint_positions):
        if self._is_active(JointWaypointMotion):
            ...
            if self.current_motion is not None:
                self.abort_motion()
            self.current_motion = JointWaypointMotion(
                [joint_positions], return_when_finished=False)
    ...
    def _inverse_kinematics(self, target_pos, target_orn):
        ...
        new_q = self.ik_solver.inverse_kinematics(
    ...
    def visualize_external_forces(self, canvas_width=500):
        ...
        for i, (lcol, lcon, f, ucon, ucol) in enumerate(zip(-collision, -contact, forces, contact, collision)):
            ...
            cv2.line(canvas, [force_bar_pos, y], [
                     force_bar_pos, y + height], thickness=4, color=[0, 0, 1])
            ucon_bar_pos = int(left + width * (ucon - lcol) / (ucol - lcol))
            cv2.line(canvas, [ucon_bar_pos, y], [ucon_bar_pos,
                     y + height], thickness=2, color=[1, 0, 0])
            lcon_bar_pos = int(left + width * (lcon - lcol) / (ucol - lcol))
            cv2.line(canvas, [lcon_bar_pos, y], [lcon_bar_pos,
                     y + height], thickness=2, color=[1, 0, 0])
